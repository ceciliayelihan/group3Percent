...
AudioPlayer car_music,drop_music;
import java.util.*;
Table table;
...
Boolean keys = false;
Boolean keya = false;
Boolean mute_music = false;
int lvl,t1;
String userName = "";
SoundFile levelup/*, car, drop*/;
User user;
CrabLegs crab;
...
void setup() {
  ...
  for(TableRow row: table.rows()){
    ...
    int user_score = row.getInt("score");
    hm.put(username,user_score);
  }
  List<Map.Entry<String, Integer>> scoreBoardList = new ArrayList(hm.entrySet());
  scoreBoardList.sort(byScore);
}

void draw(){
  //backgroundMusic.play();
  //System.out.println(backgroundMusic.isPlaying());
  if (!backgroundMusic.isPlaying()&& (mute_music ==false)){
    backgroundMusic = minim.loadFile("gamemusic.mp3");
    backgroundMusic.play();
       ...
       if (mousePressed == true) {
         ...
         for(TableRow row: table.rows()){
          ...
          if (scoreBoardList.size() < 10){
            ...
            fill(0);
            textSize(15);
            text(scoreBoardList.get(i).getKey(),250,275+i*50);
            text(scoreBoardList.get(i).getValue(),450,275+i*50);
            }
          }
          else{
            for (int i = 0; i < 10;i++){
...
////////////level///////////////////////////////////////////////////////////
  if (move==true){
    ...
    if (dist(mouseX,mouseY,35,665)<30 && mousePressed ==true && mutePressed==false) {
      ...
      mute_music = true;
    } else if (dist(mouseX,mouseY,35,665)<30 && mousePressed ==true && mutePressed==true) {
        mutePressed=false;
        backgroundMusic = minim.loadFile("gamemusic.mp3");
        backgroundMusic.play();
        mute_music = false;
    ...
    if (user.level==2){
      ...
      if (set==true){
        ...
        car1 = new Car(car_right,0,520,8,-200,70); 
        car3 = new Car(car_right,-2000,520,8,-200,70);
        car2 = new Car(car_left,800,580,-10,200,70); 
        car4 = new Car(car_left,1940,580,-10,200,70);
        set=false; }
    }
    if (user.level==3){
      imageMode(CORNER);
      ...
      if (set==true){
        ...
        car3 = new Car(car_right,-2000,520,10,-200,70);
        car2 = new Car(car_left,800,580,-10,200,70); 
        car4 = new Car(car_left,1940,580,-10,200,70);
        keys=true;
        set=false; }
    ...
    if(girltf ==true){
      ...
      if ((car1UpInteractGirl() == true) || (car2UpInteractGirl() == true) || (car3UpInteractGirl() == true) || (car4UpInteractGirl() == true)||(car1DownInteractGirl() == true) || (car2DownInteractGirl() == true) || (car3DownInteractGirl() == true) || (car4DownInteractGirl() == true)){
        ...
        car_music = minim.loadFile("car.mp3");
        car_music.play();
        user.decreaseHealth();
      }
      //boat girl interect
      if (girl.y==b0.y||girl.y==b2.y|| girl.y==b4.y){
        ...
        if (on==true){
           girl.x+=b2.speed;}
        if (on==false){
          drop_music = minim.loadFile("drop.mp3");
          drop_music.play();
          user.decreaseHealth();
          girl.x=400;
          girl.y=286;}
  ...
  if (defeat==true){
        ...
        if (replicateName() == false){
          TableRow new_row = table.addRow();//write csv data 
          new_row.setString("user",user.name);
          new_row.setInt("score",user.points);
          saveTable(table,"score.csv");
        }
        ///////////display SCORE//////////////////////////
...
Comparator<Map.Entry<String, Integer>> byScore = new Comparator<Map.Entry<String, Integer>>() {
  @Override
